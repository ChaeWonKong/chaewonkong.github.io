...
- **설치 가능**
  PWA는 설치가능하다. 물론 구글 플레이스토어나 앱스토어에서 다운로드 할 수 있는 것은 아니다. 하지만 Android 기기의 Chrome 브라우저로 PWA 사이트에 접속할 경우, 바탕화면에 설치하고 싶은지 묻는 팝업이 뜬다. 이 팝업을 통해 앱처럼 설치할 수 있다. 굳이 구글 플레이스토어에 들어갈 필요도 없기에, 오히려 유리하기도 하다.

  다만, iOS에서는 즐겨찾기를 추가하듯, 바탕화면에 추가하기를 통해서만 추가가 가능한 것은 단점이다.
